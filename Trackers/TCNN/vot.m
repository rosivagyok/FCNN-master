function [handle, image, region] = vot(format, images, groundtruth)
...
% - region (vector): Initial region encoded as a rectangle or as a polygon.
    ...
    [handle, image, region] = tracker_initialize(format, images, groundtruth);
...
function [handle, image, region] = tracker_initialize(format, images, groundtruth)
...
%     images_old = textscan(fid, '%s', 'delimiter', '\n');
%     fclose(fid);
    images{1} = images;
...
function [output] = tracker_quit(handle)
...
%
    ...
    if iscell(handle.regions)
        ...
        output = zeros(numel(handle.regions), 4);
        ...
        for i = 1:numel(handle.regions)
            ...
            elseif numel(region) == 4
                output(i,:)= [region(1) region(2) region(3) region(4)];
            elseif numel(region) >= 6 && mod(numel(region), 2) == 0
              %Louise Change fqn it only using x, y, width, height   output(i)= region(1:end);
            else
