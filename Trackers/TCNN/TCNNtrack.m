function [output] = TCNNtrack(frames, groundtruth)
...
[handle, image, region] = vot('rectangle', images, groundtruth);
...
function [state, location] = TCNN_initialize(I, region, varargin)
    ...
    % model def
    state.opts.net_file = fullfile('E:\Users\roh\p8-tracker\twostream-attempt_idontevenknowman\twostream-attempt2/TCNN/TCNN/models/imagenet-vgg-m_conv3_512-512-2.mat');
...
function [state, location] = TCNN_update(state, I, varargin)
    ...
    netc_TCNN = dagnn.DagNN().loadobj(state.net_conv);
    netc_TCNN.vars(netc_TCNN.getVarIndex('x9')).precious = 1;
    ims = gnet_extract_regions(img, samples, state.opts);
    netc_TCNN.eval({'x0',ims});
    ...
    for i=1:length(state.cand_blocks)
        ...
        net_TCNN = dagnn.DagNN().loadobj(state.blocks(b).cnn);
        net_TCNN.vars(net_TCNN.getVarIndex('x0')).precious = 1;
        net_TCNN.vars(net_TCNN.getVarIndex('x1')).precious = 1;
        net_TCNN.vars(net_TCNN.getVarIndex('x2')).precious = 1;  
        net_TCNN.vars(net_TCNN.getVarIndex('x3')).precious = 1;
        net_TCNN.eval({'x0',feat_conv});
